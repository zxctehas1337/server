...
app.post('/api/auth/verify-email', async (req, res) => {
    try {
        ...
        res.status(500).json({ success: false, error: 'Ошибка при подтверждении email' });
    }
});
...
app.post('/api/auth/resend-code', async (req, res) => {
    try {
        ...
        logger.info({ email, code: verificationCode }, 'Verification code generated');
        
        res.json({ success: true, message: 'Код подтверждения отправлен повторно' });
        
    } catch (error) {
        logger.error({ err: error }, 'Resend code error');
        res.status(500).json({ success: false, error: 'Ошибка при отправке кода' });
    }
});
...
io.on('connection', (socket) => {
    ...
    socket.on('login', async (data) => {
        try {
            ...
            const existing = await pool.query(
                'SELECT id, username, password_hash, email, email_verified, avatar_url FROM users WHERE username = $1', 
                [username]
            );
            ...
            // Update last seen
            await pool.query(
                'UPDATE users SET last_seen = CURRENT_TIMESTAMP WHERE id = $2',
                [user.id]
            );
    ...
    socket.on('authenticate_with_token', async (data) => {
        try {
            ...
            if (!token) {
                socket.emit('token_auth_success', { success: false, error: 'Token is required' });
                return;
            }
            ...
            // Find user
            const userResult = await pool.query(
                'SELECT id, username, email, email_verified, avatar_url FROM users WHERE id = $1', 
                [decoded.userId]
            );
            
            if (userResult.rows.length === 0) {
                socket.emit('token_auth_success', { success: false, error: 'User not found' });
                return;
            }
            ...
            let avatarUrl = user.avatar_url;
            if (!avatarUrl) {
                avatarUrl = `https://api.dicebear.com/7.x/avataaars/svg?seed=${encodeURIComponent(user.username.toLowerCase())}&backgroundColor=6366f1`;
                await pool.query(
                    'UPDATE users SET avatar_url = $1 WHERE id = $2',
                    [avatarUrl, user.id]
                );
            ...
            logger.info({ username: user.username, userId: user.id, socketId: socket.id }, 'User authenticated via token');
            
        } catch (error) {
            logger.error({ err: error }, 'Error during token authentication');
            socket.emit('token_auth_success', { success: false, error: 'Invalid token' });
        }
    });
